...
# run is a redeploy.
#
#   scripts/deploy.sh root@203.0.113.5                                # https://geoclustering.203-0-113-5.sslip.io
#   scripts/deploy.sh root@203.0.113.5 maps.example.com               # DNS already pointed
#
# Ported from the same script in clinical-copilot and payout-ledger, and built
# to share a server with them. Everything it creates is named after APP
# (default: this folder's name) — its directory, its Compose project and so its
# containers and network, its local port and its Caddy site file — so no
# project can overwrite another's.
#
# Simpler than theirs, because this app has no state: no database password to
# generate, no migrations, no volume, and no nightly reset. Nothing a visitor
# does is stored on the server.
#
...
#   2. writes .env.prod there with a free local port;
#   3. copies the code (rsync, so later runs send only what changed), builds
#      and starts;
...
# reload, taking every site on the server down. --force-confold keeps the
# Caddyfile below instead of the package default.
...
PORT
  ...
  remote "mkdir -p $remote_dir && cat > $remote_dir/.env.prod" <<ENV
...
ENV
  echo "wrote .env.prod (local port $free_port)"
fi
...
# is whatever this clone's .git/info/exclude keeps out of git.
rsync -az --delete --stats -e "ssh ${ssh_opts[*]}" \
  --exclude .git --exclude .env.prod --exclude .DS_Store \
